...
class Thermo_PINN():
    def __init__(self,thermal_conductivity,specific_heat_capacity,density):
        ...
        self.offset = 1000 # basically the number of points on each boundary (x=0,x=1,t=0,t=1)
        self.num_times=10000 #number of total points in simulation
        self.num_aux=2000
        self.l=1
    def run(self,end):
        PINN = PINN_Net().to(device)
        PINN_optimizer=torch.optim.Rprop(PINN.parameters(), lr=.005,etas=[.8,1.1], step_sizes=[1e-16, 100])
        ...
        for epoch in range(iterations):
            ...
            sumed_values=times*.2+x_values*1+y_values*1 #default it .1,1,1
