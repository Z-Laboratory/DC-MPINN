...
class PINN():
    ...
    def run(self,end):
        ...
        for epoch in range(iterations):
            ...
            # t_0=torch.zeros((self.baby_offset, 1), requires_grad=True).to(device) * 0
            # t_1=torch.ones((self.baby_offset,1), requires_grad=True).to(device)*end
            # t_mid=torch.rand((self.baby_offset, 1), requires_grad=True).to(device)*end

            # x_0=torch.zeros((self.baby_offset, 1), requires_grad=True).to(device) * 0
            # x_1=torch.ones((self.baby_offset,1), requires_grad=True).to(device)*self.l
            # x_mid=torch.rand((self.baby_offset, 1), requires_grad=True).to(device)*self.l

            # y_0=torch.zeros((self.baby_offset, 1), requires_grad=True).to(device) * 0
            # y_1=torch.ones((self.baby_offset,1), requires_grad=True).to(device)*self.l
            # y_mid=torch.rand((self.baby_offset, 1), requires_grad=True).to(device)*self.l

            # times=torch.cat((t_0.repeat(9,1),t_mid.repeat(9,1),t_1.repeat(9,1)))
            # intermediate_x_values=torch.cat((x_0.repeat(3,1),x_mid.repeat(3,1),x_1.repeat(3,1)))
            # x_values=intermediate_x_values.repeat(3,1)
            # intermediate_y_values=torch.cat((y_0,y_mid,y_1))
            # y_values=intermediate_y_values.repeat(9,1)


            t0=torch.zeros((self.offset0d*4+self.offset1d*4+self.offset2d*1), requires_grad=True).to(device)
            t_mid=torch.rand((self.offset1d*4+self.offset2d*4+self.offset3d*1), requires_grad=True).to(device)
            ...
            # gridx,gridy,gridt=torch.meshgrid(x,y,t,indexing='ij')
            # x_values=torch.flatten(gridx)
            # times=torch.flatten(gridt)
            # y_values=torch.flatten(gridy)
        ...
        for epoch in range(iterations):
            ...
            with torch.autograd.no_grad():
                if epoch % 100 == 0:
                    ...
                    print("end_kin",t1, torch.mean(end_kin*2*self.m),"start_kin",t2, torch.mean(start_kin*2*self.m))
    ...
    def train_AUX(self,PINN,end,old_aux):
        ...
        AUX_optimizer=torch.optim.Rprop(AUX.parameters(), lr=.01, step_sizes=[1e-16, 10])
        iterations = 101
        losses = np.zeros(iterations)
        end = end
        for epoch in range(iterations):
            ...
            normal_loss,v_loss,a_loss,stress_loss,boundary_loss,wave_loss,volume,energy=self.get_AUX_loss(aux_times,x_values,y_values,aux_outputs)
            ...
            with torch.autograd.no_grad():
                if epoch % 50 == 0:
                    ...
                    print("         V",v_loss.data,"A",a_loss.data,"stress",stress_loss.data,"boundary",boundary_loss.data,"wave",wave_loss.data,"Volume",volume.data,"energy",energy.data)
                    print("aux kin",t2, torch.mean(aux_kin*2*self.m),"prior_kin",t1,torch.mean(prior_kin*2*self.m))
                losses[epoch] = total_loss.data
        plt.clf()
        plt.yscale('log')
        plt.plot(losses)
    ...
    def get_loss(self, times, x_values, y_values, results):
        ...
        stress_DNNxx = self.k * torch.flatten(strain_x)+incompress
        stress_DNNyy = self.k * torch.flatten(strain_y)+incompress
        ...
        total_loss = boundary_loss + v_loss + a_loss + stress_loss+wave_loss+volume_loss#+energy_loss
    ...
    def get_AUX_loss(self, times, x_values, y_values,results):
        ...
        wave_loss=self.loss_function(wave_equation_x,torch.zeros_like(wave_equation_x))*1+self.loss_function(wave_equation_y,torch.zeros_like(wave_equation_y))*1
        stress_loss = self.loss_function(stress_DNNxx, sxx) * 1+self.loss_function(stress_DNNyy, syy) * 1+self.loss_function(shear_DNN, sxy) * 1
        v_loss = (self.loss_function(vx, torch.flatten(v_x))*1+self.loss_function(vy, torch.flatten(v_y))*1)*0
        a_loss = (self.loss_function(ax, torch.flatten(a_x))*1+self.loss_function(ay, torch.flatten(a_y))*1)*0
        volume=(1+strain_x)*(1+strain_y)-(strain_xy*strain_yx)
        ...
        inital_energy=torch.tensor((.5*self.k*(.5*.5+1/3*1/3))).to(device)
