...
def analytic_equations(p, *stresses):
    ...
    # s2 - s3 = c1 * (l2 ** 2 - l3 ** 2)
    # L1 * l2 * l3 = 1
    return (2*c1 * (l1 ** 2 - l2 ** 2)-2*c2*(1/(l1**2)-1/(l2**2)) - s1 + s2, 2*c1 * (l1 ** 2 - l3 ** 2)-2*c2*(1/(l1**2)-1/(l3**2)) - s1 + s3, l1 * l2 * l3 - 1)

def dist(s1,s2,s3,x,y,z):
    a = (y)
    b=torch.ones_like(a)
    c=torch.zeros_like(a)
    distance_function =torch.cat((x, y ,z,1-x, 1-y, 1-z, c, c,c,b), dim=1)
...
def get_BC(s1,s2,s3,x,y,z):
    ...
    b=torch.ones_like(a)
    c=torch.zeros_like(a)
    BC = torch.cat((x, y ,z,s1, s2, s3, c, c,c,b), dim=1)
    return BC
...
def f(t,s1,s2,s3, x, y, z, net, epoch,points):
    ...
    l3=F[:,2,2]
    I1=l1**2+l2**2+l3**2
    I1=torch.reshape(I1,(points,1,1))
    #I2=l1*l2+l1*l3+l2*l3

    #stress = torch.mul(B,I1) #2*(c1+I1*c2)*B#-2*c2*B*B- eye*p
    stress=2*(c1+I1*c2)*B-2*c2*B*B- eye*p
...
def train_PDE(net, optimizer,optimizer_version):  # ,mse_cost_function,x_bc,t_bc,u_bc):
    ...
    for epoch in range(iterations):
        ...
        y = np.random.uniform(-0.1, 1.1, num_collocation)
        ...
        # stress3=np.ones_like(stress1)*0
...
if __name__ == "__main__":
    ...
    with open('MR_pinn.csv', 'w', newline='') as file:
        ...
        while time_to_evaluate<=1.001:
            ...
            if setup:
                ...
                if trainPDE:
                    ...
                    torch.save(net.state_dict(), "PDE_MR.pt")
                    plt.plot(losses)
                    # plt.yscale('log')
                ...
                net.load_state_dict(torch.load("PDE_MR.pt"))
            ...
            if single_eval == True:
                ...
                print("analytical solution", l1, l2, l3)
                c1 = .3
                c2 = .032
                l1=1.66557
                print("theoretical solution", ((2 * c1 + 2 * c2 / l1) * (l1 **2 - 1 / l1)))
                quit()
            plt.close()
            setup=False
        ...
        c1=.2
        c2=.05
        print("theoretical solution",((2*c1+2*c2/l1)*(l1*l1-1/l1)))
